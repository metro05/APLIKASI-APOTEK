...
if (isset($_POST['login'])) {
  ...
  $password = md5($_POST['password']); // Gunakan fungsi md5() untuk mengenkripsi password
  ...
  if ($cek > 0) {
    $dt_user = mysqli_fetch_array($data);
    $_SESSION['user'] = $username;
    $_SESSION['level'] = $dt_user['level'];
    $_SESSION['login'] = true;
    header('location:index.php');
    exit(); // Tambahkan exit() setelah header untuk menghentikan eksekusi kode
  } else {
    echo "<div class='alert alert-danger'>Maaf !! Anda gagal login</div>";
  }
}
?>
<!DOCTYPE html>
<html>
  <head>
    <meta charset="UTF-8">
    <title>Login | Aplikasi Penjualan Obat</title>
